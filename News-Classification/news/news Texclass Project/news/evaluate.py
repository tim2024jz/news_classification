...
def semeval_scorer(predict_label, true_label, class_num=10):
    # Function to calculate F1 score for SemEval task
    import math
    ...
    for i in range(1, class_num):
        if f1[i] > 0.0:  # classes that are not in the predict label are not considered
            actual_class += 1
            total_f1 += f1[i]
    try:
...
class Eval(object):
    def __init__(self, config):
        # Initialize with the device configuration
